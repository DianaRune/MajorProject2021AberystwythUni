...
public class MusicManager implements MusicGen {
    ...
    public static void playMusic() throws MidiUnavailableException {
        ...
        while (true) {
            ...
            while (isPlaying) {
                ...
                //(Will attempt to generate and play notes.)
    ...
    public static void generateMusic(Note[][][] tracks, MidiChannel[][] midiChannels, double[][] lastTimeNotePlayed) {
        ...
        for (int i = 0; i < tracks.length; i++) {
            ...
            for (int j = 0; j < (tracks[i].length); j++) {
                ...
                //The default tempo (120bpm/500ms) is set here. It is used to divide the current tempo.
                double baseTempo120 = 500;
                //When this decimal is multiplied by the standard duration of a note...
                double tempoFactor = tempo/baseTempo120;
                //Then the (note duration / type of note(or bottom half of the time signature)) is appropriately adjusted to suit the tempo.
                //If this value is greater than the time the note was played last then...
                if (((tracks[i][j][0].duration / timeSignatureBottom) * tempoFactor) <= (System.currentTimeMillis() - lastTimeNotePlayed[i][j]))
                {
    ...
    //This method pairs the selected voice values to their respective tracks, for calling, in an array.
    public void setTimeSig(int timeSigTop, int timeSigBot) {
    ...
    public void setOctave(int octaveValue){
        octave = octaveValue;
    }

    //This method sets the system's current tempo.
    public void setTempo(int tempoTime) {
        tempo = tempoTime;
    }
    //##################################################################################################################
    ...
    //This method is called when the user wants to play a previous song.
    public void playPreviousSong() {
        //This requests for the previous song's seed to be accessed and assigned.
        //(If successful and the song changes, 'true' is returned, satisfying 'changingSong(boolean)'.)
        changingSong(SongSeedGenerator.playPreviousSeed());
    }
    ...
    public void playNextSong() {
        ...
        //(If successful and the song changes, 'true' is returned, satisfying 'changingSong(boolean)'.)
        changingSong(SongSeedGenerator.playNextSeed());
    }

    //This method is called when a song change is attempted...
    //It adjusts/updates the system's time stamps and current song details.
    public static void changingSong(boolean skipSuccessful)
    {
        //If the song skip was successful then the update occurs...
        if (skipSuccessful) {
    ...
    public boolean switchPlayPause() {
        ...
        isPlaying = !isPlaying;
        //return whether the system is playing or not.
        return isPlaying;
    }
}